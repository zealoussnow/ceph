...
class CRUSHMap(ceph_module.BasePyCRUSH):
    ...
    def get_pool_osds(self, item):
        return self._get_pool_osds(item)
...
class MgrModule(ceph_module.BaseMgrModule):
    ...
    def get_osd_lat(self, daemon_type, daemon_name, stat):
        data = self.get_counter(daemon_type, daemon_name, stat)[stat]

        #self.log.error("get_latest {0} data={1}".format(stat, data))
        if data and len(data) > 1:
            count = data[-1][2] - data[-2][2]
            if not count:
                return 0
            return (data[-1][1] - data[-2][1]) / float(count)
        else:
            return 0

    def get_osd_lat_sum(self, daemon_type, daemon_name, stat):
        data = self.get_counter(daemon_type, daemon_name, stat)[stat]
    ...
    def get_osd_count_sum(self, daemon_type, daemon_name, stat):
        data = self.get_counter(daemon_type, daemon_name, stat)[stat]
