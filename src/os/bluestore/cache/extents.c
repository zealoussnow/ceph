...
static const char *
bch_ptr_status(struct cache_set *c, const struct bkey *k)
{
  unsigned i;
  for (i = 0; i < KEY_PTRS(k); i++) {
    if (ptr_available(c, k, i)) {
...
bch_bkey_dump(struct btree_keys *keys, const struct bkey *k)
{
  ...
  for (j = 0; j < KEY_PTRS(k); j++) {
    ...
    if (n >= b->c->sb.first_bucket && n < b->c->sb.nbuckets)
      printf(" prio %i", PTR_BUCKET(b->c, k, j)->prio);
  }
...
bch_extent_sort_fixup(struct btree_iter *iter,struct bkey *tmp)
{
  ...
  while (iter->used > 1) {
    ...
    if (iter->used > 2 && bch_extent_sort_cmp(i[0], i[1])) {
      i++;
    }
  ...
  while (1) {
    ...
    ************************/
    if (bkey_cmp(k, &START_KEY(insert)) <= 0) {
      // insert 在k后面，接着迭代下一个k1
      continue;
    }
    // 存在重叠，说明等会儿会更改原来的bkey k，所以先记录下
    // k最开始的offset和size
    ...
    */
    if (replace_key && KEY_SIZE(k)) {
      ...
      */
      unsigned i;
      uint64_t offset = KEY_START(k) - KEY_START(replace_key);
    ...
    ************************/
    ...
    if (bkey_cmp(insert, k) < 0 &&
      ...
      */
      struct bkey *top;
      // 整个包含，重叠部分就是insert的范围，即(offset-len, len)
      bch_subtract_dirty(k, c, KEY_START(insert),KEY_SIZE(insert));
      if (bkey_written(b, k)) {
        /*
...
out:
  if (KEY_DIRTY(insert)) {
    bcache_dev_sectors_dirty_add(c, KEY_INODE(insert), KEY_START(insert),
                                KEY_SIZE(insert));
  }
  return false;
}
