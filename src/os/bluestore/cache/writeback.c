...
static void __update_writeback_rate(struct cached_dev *dc)
{
	...
	change = proportional + derivative;
        ...
        printf("dirty ==== %d, derivative = %d, target = %d change = %d\n", dirty, derivative, target, change);
...
static void update_writeback_rate(void *arg)
{
	/*struct cached_dev *dc = container_of(to_delayed_work(work),*/
        ...
        pthread_setname_np(pthread_self(), "fuckzhang_ur");
...
static void dirty_io_complete(struct keybuf_key *w, struct cached_dev *dc)
{
        if (KEY_DIRTY(&w->key)) {
                ...
                /*bch_keylist_add(&keys, &KEY(0, KEY_OFFSET(&w->key), KEY_SIZE(&w->key)));*/
...
static void read_dirty(struct cached_dev *dc)
{
	...
	while (!dc->writeback_should_stop) {
                ...
                printf("<%s>: bkey ptr_offset = %lu, offset=%lu,size=%lu  dirty == %d\n", __func__, PTR_OFFSET(&w->key, 0), KEY_OFFSET(&w->key), KEY_SIZE(&w->key), KEY_DIRTY(&w->key));
...
static int bch_writeback_thread(void *arg)
{
	...
	bool searched_full_index;
        ...
        pthread_setname_np(pthread_self(), "fuckzhang_wb");
	while (!dc->writeback_should_stop) {
